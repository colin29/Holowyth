package com.wizered67.vnlib.conversations.commands.impl.audio;
...
public class PlayMusicCommand implements ConversationCommand {
    ...
    public void execute(ConversationController conversationController) {
        if (type == Type.PAUSE) {
            VNHubManager.musicManager().pauseMusic(index);
            return;
        } else if (type == Type.RESUME) {
            VNHubManager.musicManager().resumeMusic(index);
            return;
        }
        if (music.equals("")) {
            VNHubManager.musicManager().stopMusic(index);
            return;
        }
        VNHubManager.musicManager().playMusic(music, loops, volume, index);
    }
