...
public abstract class SceneEntity implements Comparable<SceneEntity> {
     ...
     * If the image is actually a texture the animation just contains a single frame
     * */
    ...
    public void reload() {
        currentAnimation = VNHubManager.assetManager().getAnimationOrTextureAsAnimation(animationName);
        updateSprite();
    }
    ...
    /** Switches this Entity's texture/animation to the one named NAME. Returns true if animation is valid.
     * Note: currentAnimation can simply be a texture too
     *  */
    public boolean setCurrentAnimation(String identifier) {
        if (!animationName.equalsIgnoreCase(identifier)) {
            stateTime = 0;
            animationName = identifier;
        }
        currentAnimation = VNHubManager.assetManager().getAnimationOrTextureAsAnimation(identifier);
        if (currentAnimation == null)
        	logger.error("No texture/animation resource found with id '{}'", identifier);
        wasFinished = false;
        //update(0);
        /*
        if (!isVisible()) {
