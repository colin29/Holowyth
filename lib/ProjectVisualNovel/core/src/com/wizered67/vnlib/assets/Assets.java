package com.wizered67.vnlib.assets;
...
import com.wizered67.vnlib.assets.parameters.ParametersLoader;
import com.wizered67.vnlib.conversations.Conversation;
import com.wizered67.vnlib.conversations.scene.SceneCharacter;
import com.wizered67.vnlib.conversations.scene.SceneManager;
import com.wizered67.vnlib.conversations.xmlio.ConversationAssetLoader;
...
public class Assets {
    ...
    public static final String ASSETS_DIR = "vnassets/";
    private final String RESOURCE_CONFIG_XML = "Resources.xml";

    private final String RESOURCE_DIRECTORIES_FILE = ASSETS_DIR + "ResourceDirectories.xml";
    private final String CHARACTERS_FILE =  ASSETS_DIR + "CharacterDefinitions.xml";
    private final String LOADING_FILE = ASSETS_DIR + "LoadingGroups.xml";
    ...
    public Animation<TextureRegion> getAnimationOrTextureAsAnimation(String identifier) {
    ...
    private void loadAllResourceDefinitions(Element directoriesRoot) {
        for (int i = 0; i < directoriesRoot.getChildCount(); i++) {
            ...
            try {
                ...
                Element resourcesRoot = xmlReader.parse(Gdx.files.internal(ASSETS_DIR + directoryName + "/" + RESOURCE_CONFIG_XML));
	...
	private void loadResourceDefinitions(String directory, Element resourcesRoot, Class type) {
        for (int i = 0; i < resourcesRoot.getChildCount(); i++) {
            ...
            String name = resourceElement.getAttribute("name"); // this is 'name' which is still different than 'identifier' which is used in convo.
            String filepath =  ASSETS_DIR + directory + "/" + name; // On windows, which we are targeting, Gdx.files.internal looks up relative to project root, thus we include ASSETS_DIR  
            if (!Gdx.files.internal(filepath).exists()) {
                logger.error("No such file {} (name {})", filepath, name);
                continue;
            }
            logger.debug("Loaded Resource definition: {}: ({}, {})", type.getSimpleName(), name, filepath);
            ...
            assetIdentifiers.put(identifier, new AssetDescriptor(filepath, type, parameters));
        }
    }
        ...
        for (int c = 0; c < groups.getChildCount(); c += 1) {
            ...
            for (int a = 0; a < group.getChildCount(); a += 1) {
                ...
                if (asset.getName().equals("load")) { //todo use constant for name?
                    ...
                    if (descriptor == null) {
                        logger.error("No such resource with name {}", identifier);
                        continue;
                    }
    ...
    private static void identifierError(String id) {
        VNHubManager.error("Identifier '" + id + "' is already in use.");
    }
    ...
    public void loadGroup(String groupName) {
        ...
        if (group == null) {
            VNHubManager.error("No such group with name " + groupName);
            return;
        }
